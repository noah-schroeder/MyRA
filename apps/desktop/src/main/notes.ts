...
export async function chat(opts: ChatOptions): Promise<string> {
  const { endpoint } = opts;
  if (!endpoint.baseUrl) {
    throw new NotesError("No LLM endpoint is configured. Set one in Settings → Endpoints.");
  }
  ...
  let res: Response;
  try {
    res = await appFetch(chatUrl(endpoint.baseUrl), {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        // Extraction is not a creative task, and a warm model invents owners.
    ...
    if (err instanceof EgressBlocked) throw new NotesError(err.message);
    if ((err as Error).name === "TimeoutError" || (err as Error).name === "AbortError") {
      throw new NotesError(
        `The LLM endpoint did not answer within ${(endpoint.timeoutMs || 120_000) / 1000}s. ` +
          `A long meeting may simply need a longer timeout in Settings.`,
      );
    }
    throw new NotesError(`Could not reach the LLM endpoint at ${endpoint.baseUrl}: ${(err as Error).message}`);
  }

  if (!res.ok) {
    const body = (await res.text().catch(() => "")).slice(0, 400).trim();
    if (res.status === 401 || res.status === 403) {
      throw new NotesError(`The LLM endpoint rejected the API key (${res.status}). Check it in Settings.`);
    }
    throw new NotesError(`The LLM endpoint failed: ${res.status} ${res.statusText}${body ? ` — ${body}` : ""}`);
  }

  const body = (await res.json().catch(() => undefined)) as
    | { choices?: { message?: { content?: unknown } }[]; error?: { message?: string } }
    | undefined;
  if (body?.error?.message) throw new NotesError(body.error.message);
  const content = body?.choices?.[0]?.message?.content;
  if (typeof content !== "string" || !content.trim()) {
    throw new NotesError("The LLM endpoint returned an empty reply.");
  }
  return content;
}
