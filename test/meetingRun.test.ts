...
import { defaultTracks } from "../src/core/meetings/meeting.ts";
import { duration, noteName, renderReport, renderTranscript } from "../src/core/meetings/meetingRun.ts";
import type { MeetingRecord } from "../src/core/meetings/meeting.ts";
import type { MeetingNotes } from "../src/core/meetings/notes.ts";

const sink = { id: "out-1", name: "display", description: "Built-in Audio", kind: "system" as const };
...
test("the microphone alone is not enough for a remote meeting", () => {
  ...
  assert.equal(both[1]!.source, "out-1");
