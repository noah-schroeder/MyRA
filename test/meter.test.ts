...
} from "../src/core/meetings/meter.ts";
...
/*
 * The plumbing, against a real recorder.
 *
 * The maths above can be right while the plumbing is wrong -- reading the
 * header as audio, or never advancing the window -- and neither shows up in a
 * pure test. v1 tested this by recording from a live microphone, which meant
 * the test could only run on a Linux box with PipeWire and could never assert
 * a level, because whether the machine hears anything is not up to the test.
 *
 * Now that the capture side pushes audio in rather than a subprocess pulling
 * it, the test can supply known audio and assert the actual value.
 */
test("levels can be read from a recording as it is being written", async () => {
  const { Recorder } = await import("../src/core/meetings/capture.ts");
  const recorder = new Recorder();
  await recorder.start({});
  try {
    assert.equal(await recorder.level(), undefined, "a silent start must read as no signal, not zero");

    await recorder.write(tone(0.5));
    const half = await recorder.level();
    assert.ok(half, "no reading after audio arrived");
    assert.ok(Math.abs(half.peak - 0.5) < 0.01, `peak ${half.peak} should be ~0.5`);
    assert.ok(half.rms > 0 && half.rms < half.peak, `rms ${half.rms} must sit under peak ${half.peak}`);

    // The window must advance, or the meter freezes on the first chunk.
    await recorder.write(tone(0.05, 16_000));
    const quiet = await recorder.level();
    assert.ok(quiet && quiet.peak < half.peak, `level did not fall: ${quiet?.peak} vs ${half.peak}`);

    const done = await recorder.stop();
    assert.ok(done.seconds > 0.9, `expected ~1.1s of audio, got ${done.seconds}`);
    assert.equal(done.bytes, 44 + 1600 * 2 + 16_000 * 2, "header plus every sample written");
  } finally {
...
/*
 * A WAV whose header says zero is a WAV that plays nothing, and v1 depended on
 * SIGTERM (never SIGKILL) reaching pw-record for the header to be finalised.
 * Owning the file removes that dependency, so assert the guarantee directly.
 */
test("a finished recording carries its real length in the header", async () => {
  const { Recorder } = await import("../src/core/meetings/capture.ts");
  const { readFile } = await import("node:fs/promises");
  const recorder = new Recorder();
  await recorder.start({});
  try {
    await recorder.write(tone(0.4, 8_000));
    const done = await recorder.stop();
    const header = (await readFile(done.path)).subarray(0, 44);
    assert.equal(header.subarray(0, 4).toString("ascii"), "RIFF");
    assert.equal(header.subarray(8, 12).toString("ascii"), "WAVE");
    assert.equal(header.readUInt32LE(40), 8_000 * 2, "data chunk length");
