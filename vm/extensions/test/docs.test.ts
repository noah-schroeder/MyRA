...
test("scratch space is inside the workspace, never /tmp", async () => {
  ...
  const { scratchRoot, workspaceRoot, documentsDir } = await import("../docs/office.ts");
...
test("the documents folder is the only place these tools reach", async () => {
  const { inWorkspace, documentsDir, DocsError } = await import("../docs/office.ts");
