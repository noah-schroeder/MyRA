...
import { buildScreenPrompt, parseDecisions, type Candidate } from "../research/screen.ts";
import { locateClaims } from "../research/extract.ts";
import { extractJson, parseJsonReply, buildArgs } from "../research/subagent.ts";
import { findVerbatim } from "../research/sources.ts";
...
test("subagent stages run sealed off by default", () => {
  const args = buildArgs({ model: "p/m", prompt: "go" });
  // A stage that could load this very extension, or read an AGENTS.md, is a
  // stage whose judgement can be changed by something outside the run.
  for (const flag of ["--no-tools", "--no-extensions", "--no-skills", "--no-context-files", "--no-session"]) {
    assert.ok(args.includes(flag), `expected ${flag}`);
  }
  assert.deepEqual(args.slice(-1), ["go"]);
  assert.ok(args.includes("--tools") === false);
});

test("unassigned roles fall back to the model the user is already using", async () => {
  const { resolveRoles, reviewerIsSynthesist } = await import("../research/roles.ts");
...
test("a role config with a bare model id is ignored, not half-used", async () => {
  const { readRoleConfig } = await import("../research/roles.ts");
...
test("the embeddings endpoint survives the trip through research.json", async () => {
  const { readResearchConfig } = await import("../research/config.ts");
  const { writeFileSync, mkdtempSync } = await import("node:fs");
  const { join } = await import("node:path");
  const { tmpdir } = await import("node:os");
...
test("a missing embeddings key is reported as a key problem, not a network one", async () => {
  const { configuredEmbeddingEndpoint } = await import("../research/embed.ts");
  const { writeFileSync, mkdtempSync } = await import("node:fs");
  const { join } = await import("node:path");
  const { tmpdir } = await import("node:os");
...
test("screening batches are worked out before any of them runs", async () => {
  const { batchRanges } = await import("../research/screen.ts");
...
test("the idle timeout formats its own limit in readable units", async () => {
  // Reported as "0 minutes" for a six-second limit before this was fixed.
  const { runSubagent } = await import("../research/subagent.ts");
  await assert.rejects(
    () => runSubagent({ model: "definitely/not-a-real-binary-path", prompt: "x", idleTimeoutMs: 1 }),
    (err: Error) => !/0 minutes/.test(err.message),
  );
});
