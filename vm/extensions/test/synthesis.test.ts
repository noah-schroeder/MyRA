...
import { buildSynthesisPrompt, formatClaims, DanglingCitationError } from "../research/synthesize.ts";
import {
  buildVerifyPrompt, pairsToCheck, parseVerdicts, splitCitedSentences, summarizeChecks,
} from "../research/verify.ts";
import { buildRevisionPrompt, formatFlags } from "../research/review.ts";
import { auditCitations, makeSourceRecord, type SourceRecord } from "../research/sources.ts";
import type { Claim } from "../research/extract.ts";
...
test("the PRISMA funnel and summary are derived from the files, not tracked", async () => {
  const { ResearchRun } = await import("../research/run.ts");
