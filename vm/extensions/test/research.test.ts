...
import { canonicalUrl, htmlToText } from "../research/html.ts";
import { dedupe } from "../research/searxng.ts";
import { abstractFromInverted, venueOf } from "../research/openalex.ts";
import { parseArxivEntries } from "../research/arxiv.ts";
...
test("dedupe keeps first-seen order and drops repeats", () => {
  ...
  assert.deepEqual(out.map((h) => h.title), ["A", "B"]);
});
...
import { stubApi } from "./harness.ts";
import { readResearchConfig } from "../research/config.ts";

/**
 * Re-import the extension with a fresh module registry entry.
 *
 * The module resolves its config path once at load, so each mode needs its own
 * evaluation. The specifier is built through a variable deliberately: a literal
 * would be statically resolved and the query string rejected.
 */
async function loadExtensionFresh(tag: string): Promise<{ default: (pi: unknown) => void }> {
  const spec = new URL(`../research/index.ts?${tag}`, import.meta.url).href;
  return (await import(spec)) as { default: (pi: unknown) => void };
}
...
test("the mode button really removes the other research tool", async () => {
  const all = ["read", "bash", "fetch_page", "web_search", "deep_research", "academic_research"];

  for (const [mode, expected, gone] of [
    ["web", "web_search", "deep_research"],
    ["deep", "deep_research", "web_search"],
  ] as const) {
    const path = configFile({ mode, category: "science" });
    process.env["KAREN_RESEARCH_CONFIG"] = path;
    // Re-import with the env var set: the module resolves its config path once.
    const mod = await loadExtensionFresh(`mode=${mode}`);
    const s = stubApi(all);
    mod.default(s.pi);
    await s.handlers.get("session_start")?.({}, {});

    const active = s.activeTools();
    ...
    assert.ok(active.includes("read") && active.includes("bash") && active.includes("fetch_page"),
...
test("off leaves every research tool available", async () => {
  const path = configFile({ mode: "off", category: "general" });
  process.env["KAREN_RESEARCH_CONFIG"] = path;
  const mod = await loadExtensionFresh("mode=off");
  const s = stubApi(["read", "web_search", "deep_research", "academic_research"]);
  mod.default(s.pi);
  await s.handlers.get("session_start")?.({}, {});
  for (const t of ["web_search", "deep_research", "academic_research"]) {
    assert.ok(s.activeTools().includes(t), `${t} should be available in off mode`);
  }
  delete process.env["KAREN_RESEARCH_CONFIG"];
});

test("parseCategories splits SearXNG's comma form and drops blanks and repeats", async () => {
  const { parseCategories } = await import("../research/config.ts");
...
test("a multi-category selection survives a round trip through the plan document", async () => {
  const { renderPlan, parsePlan } = await import("../research/plan.ts");
  const plan = {
...
test("the GUI's time range wins over the model's, and 'any time' clears it", async () => {
  const { effectiveTimeRange } = await import("../research/config.ts");
